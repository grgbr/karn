...
class SlistPerfResultPlotter:
	...
	def plot_presort(self, dir_path, presort_name, interactive):
		for presort, name in enumerate(self._agg.presorts()):
			if name == presort_name:
				break

		for p in range(0, len(self._agg.presorts())):
			...
			results = self._agg.results_bypresort(presort)
			max_keys = 0
			for a in range(0, len(results)):
				keys = [r.keynr for r in results[a]]
				if keys[-1] > max_keys:
					max_keys = keys[-1]
				times = [r.average_time() * 1e-3
				         for r in results[a]]
				plt.plot(keys, times, 'o-', alpha = 0.6,
				         label = str(self._agg.algorithms()[a]))
			...
			# Display legend for each plot in the upper right corner
			plt.legend(loc = 'upper right')
			# Set main title
			plt.title(presort_name +
			          ' presort scheme performance by sorting ' +
			          'algorithm')
...
if __name__ == '__main__':
	...
	plot.plot_presort(dir_path, args.presort[0], args.interactive)
